...
- **信任设备。** 确认时勾选“信任此设备”，以后这台设备发来的文件直接保存；可在设置中移除，也可开启“只接收已信任设备”。
- **发送文件夹和文字。** 文件夹保留目录结构（跳过符号链接和 .DS_Store 等系统文件）；文字和链接对方可一键复制或打开。
...
- 首次连接为“首次信任”模型：未信任设备的请求需要用户确认，确认框显示对方指纹，可与对方设置页的本机指纹核对。
